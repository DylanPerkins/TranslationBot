...
class TranslateMessage(commands.Cog):
    def __init__(self, bot):
        ...
        self.bot.tree.add_command(app_commands.ContextMenu(
            ...
            callback=self.translate_to_spanish
        ))

        self.bot.tree.add_command(app_commands.ContextMenu(
            name="Translate to Chinese",
            callback=self.translate_to_chinese
        ))

        self.bot.tree.add_command(app_commands.ContextMenu(
            name="Translate to French",
            callback=self.translate_to_french
        ))

        self.bot.tree.add_command(app_commands.ContextMenu(
            name="Translate to Ukrainian",
            callback=self.translate_to_ukrainian
        ))


    @commands.hybrid_command()
    async def translate(self, ctx: commands.Context, message_id: str, language: LanguageChoices = None):
        ...
        try:
            ...
            if not message_id.isdigit():
                return await ctx.send("Please enter a valid message ID.", ephemeral=True)
            ...
            await ctx.send(f"### Orginal message:\n{content}\n### Translated message to __{language_name}__:\n{translated_text}", delete_after=300, silent=True)

        except discord.errors.NotFound:
            await ctx.send(f"Sorry, I couldn't find a message with that ID (`{message_id}`) in this channel.", ephemeral=True)
        except Exception as e:
            await ctx.send(f"Whoops! An error occurred while translating the message:\n{str(e)}", ephemeral=True)
    ...
    async def translate_to_spanish(self, interaction: Interaction, message: discord.Message):
        content = message.content
        auth_key = self.bot.config.deepl_auth_key
    ...
    async def translate_to_chinese(self, interaction: discord.Interaction, message: discord.Message):
        content = message.content
        auth_key = self.bot.config.deepl_auth_key
    ...
    # Translate to French
    async def translate_to_french(self, interaction: discord.Interaction, message: discord.Message):
        content = message.content
        auth_key = self.bot.config.deepl_auth_key
    ...
    async def translate_to_ukrainian(self, interaction: discord.Interaction, message: discord.Message):
        content = message.content
        auth_key = self.bot.config.deepl_auth_key
